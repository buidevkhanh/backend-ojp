        ...
        return __generator(this, function (_a) {
            switch (_a.label) {
                case 0: return [4 /*yield*/, user_repository_1.UserRepository.findOneByCondition({
                        userEmail: email,
                        userRole: app_object_1.AppObject.ROLES.ADMIN,
                    })];
                case 1:
                    existAdmin = _a.sent();
                    if (!existAdmin) {
                        throw new app_error_1.AppError("PermissionDenied", 403);
                    }
                    password = (0, string_util_1.hashInformation)(password);
                    if (password === existAdmin.userPass) {
                        return [2 /*return*/, {
                                token: jwt_1.default.signToken({ userOrEmail: existAdmin.username, role: existAdmin.userRole }, environment_1.envConfigs.REFRESH_TOKEN_EXPIRED),
                            }];
                    }
                    else {
                        throw new app_error_1.AppError("InvalidPassword", 400);
                    }
                    return [2 /*return*/];
            }
        });
